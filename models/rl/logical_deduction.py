...
from models.filters.logical_deduction import ParsedAttempt_LogicalDeduction, ParsedFeedback_LogicalDeduction
from models.self_improve.logical_deduction import LLMwVerifier_LogicalDeduction
import re


class LLMEditor_LogicalDeduction(LLMwVerifier_LogicalDeduction, LLMEditor):
    FB_ERROR  = '[ERROR]'
    FB_NOEDIT = '[NOEDIT]'
    ...
    def __init__(
        ...
        self.additional_info = """Let "??" represents 0 or more objects, and "?" represents exactly 1 object."""
        self.log = []
        return
    
    def _check_attempted_answer(self, llm_output, prev_answer, prev_feedback):
        attempted_answer = llm_output['attempted_answer']
        parsed_attempt = ParsedAttempt_LogicalDeduction(attempted_answer)
        parsed_prev_feedback = ParsedFeedback_LogicalDeduction(prev_feedback)
    ...
    def get_filtered_verifier_feedback(self, question, attempt, correct_answer):        
        ...
        parsed_attempt = ParsedAttempt_LogicalDeduction(attempt)
        if correct_answer.lower() == final_chioce.lower() and parsed_attempt.has_valid_content():
        ...
        parsed_feedback = ParsedFeedback_LogicalDeduction(generated_feedback)
    ...
    def get_to_edit_feedback(self, attempts: List[str], gen_feedbacks: List[str], llm_feedbacks: List[str], correct_ans: float):
        for gen_attempt, gen_feedback, llm_feedback in zip(attempts, gen_feedbacks, llm_feedbacks):
            ...
            parsed_llm_feedback = ParsedFeedback_LogicalDeduction(llm_feedback)
            parsed_gen_feedback = ParsedFeedback_LogicalDeduction(gen_feedback)
    ...
    def get_llm_answer_until_done(self, llm_prompt, correct_ans: float):
        ...
        while not done and updated < self.max_updates:
            ...
            parsed_attempted_answer = ParsedAttempt_LogicalDeduction(attempted_answer)
            ...
            parsed_attempt_w_question = ParsedAttempt_LogicalDeduction(attempted_answer_w_question)
            ...
            parsed_feedback = ParsedFeedback_LogicalDeduction(feedback)
            parsed_prev_feedback = ParsedFeedback_LogicalDeduction(prev_feedback)
        ...
        for p, p_ans, c_ans in zip(preds, pred_ans, correct_ans):
            try:
                ...
                if p is None:
                    ...
                    continue
                if p_ans == c_ans and ParsedAttempt_LogicalDeduction(p).has_valid_content():
                    ori_p = p
