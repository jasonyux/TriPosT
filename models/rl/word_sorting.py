...
from models.filters.word_sorting import ParsedAttempt_WordSorting, ParsedFeedback_WordSorting
from models.verifier.base import ScriptedVerifier
from models.self_improve.word_sorting import LLMwVerifier_WordSorting

import re


class LLMEditor_WordSort(LLMwVerifier_WordSorting, LLMEditor):
    FB_ERROR  = '[ERROR]'
    FB_NOEDIT = '[NOEDIT]'
    ...
    def _get_prev_partial_answer(self, prev_answer, prev_feedback:ParsedFeedback):
        ...
        error_step_int = 0
        # so if the first step is wrong, error_step_int=0
    ...
    def _check_attempted_answer(self, llm_output, prev_answer, prev_feedback):
        ...
        parsed_attempt = ParsedAttempt_WordSorting(attempted_answer)
        parsed_prev_feedback = ParsedFeedback_WordSorting(prev_feedback)
    ...
    def get_filtered_verifier_feedback(self, question, attempt, correct_answer):        
        # generate feedback using a scripted verifier
        if isinstance(self.verifier_model, ScriptedVerifier):
            verifier_input = {
                'question': question,
                'attempted_answer': attempt,
                'correct_answer': correct_answer,
            }
            output = self.verifier_model.generate(verifier_input, out_dict=True)
            ...
            try:
                ...
                last_step_num = re.findall(r'^\((.+)\) \w.+', second_last_step)[0]
                if correct_answer == final_sorted_words:
                    return f"Step (1) to step ({last_step_num}) are correct. The final response is also correct."
            except:
                print_red(f'cannot find final answer sorting or step number:')
                print_red(f'last_step: {last_step}')
                print_red(f'second_last_step: {second_last_step}')
                return "[ERROR]"
            ...
            generated_feedback = output['feedback']
            parsed_feedback = ParsedFeedback_WordSorting(generated_feedback)
    ...
    def get_llm_feedbacks(self, question: str, attempts: List[str], correct_ans: list):
        llm_feedbacks = []
        for attempt in attempts:
            attempt = attempt.strip()
            feedback = self.get_filtered_verifier_feedback(question, attempt, correct_ans)
            llm_feedbacks.append(feedback)
        return llm_feedbacks

    def get_to_edit_feedback(self, attempts: List[str], gen_feedbacks: List[str], llm_feedbacks: List[str], correct_ans: list):
        for gen_attempt, gen_feedback, llm_feedback in zip(attempts, gen_feedbacks, llm_feedbacks):
            ...
            parsed_llm_feedback = ParsedFeedback_WordSorting(llm_feedback)
            parsed_gen_feedback = ParsedFeedback_WordSorting(gen_feedback)
    ...
    def get_llm_answer_until_done(self, llm_prompt, correct_ans: list):
        llm_output = {}
        updated = 0
        ...
        while not done and updated < self.max_updates:
            ...
            parsed_attempted_answer = ParsedAttempt_WordSorting(attempted_answer)
            ...
            if "ERROR" in feedback:
                break
            done = self.check_if_done(feedback) and parsed_attempted_answer.has_valid_content()

            parsed_feedback = ParsedFeedback_WordSorting(feedback)
            parsed_prev_feedback = ParsedFeedback_WordSorting(prev_feedback)
    ...
    def _post_process_feedback(self, feedback: str):
        # no need to _post_process_feedback since it is scripted
    ...
    def format_trajectory(self, trajectory: List[str]):
        ...
        for step in trajectory[3:]:
            ...
            if is_attempt:
                ...
                cleaned_feedback = self._post_process_feedback(step)
                out_text += f"Feedback: {cleaned_feedback}\n"
            is_attempt = not is_attempt
        return out_text.strip()

    def _edit_single(self, pred: str, pred_ans: list, correct_ans: list):
        # word sorting questions have two lines
        question = '\n'.join(pred.split('\n')[:2])
    ...
    def fix_question_mark_no_space(self, pred: str):
        pred = re.sub(r'(["])(\?)', r'\1 ?', pred)
        return pred

    def edit(self, preds: List[str], pred_ans: List[list], correct_ans: List[list]):
        edited_traj = []
        for p, p_ans, c_ans in zip(preds, pred_ans, correct_ans):
            try:
                ...
                p_ori = p
                p = self.clean_pred(p)
                p = self.fix_question_mark_no_space(p)
                if self.verbose:
                ...
                if p is None:
                    ...
                    continue
                if p_ans == c_ans and ParsedAttempt_WordSorting(p).has_valid_content():
                    ori_p = p
