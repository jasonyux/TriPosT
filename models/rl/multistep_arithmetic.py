...
from models.filters.multistep_arithmetic import ParsedAttempt_MultistepArithmetic, ParsedFeedback_MultistepArithmetic
from models.verifier.base import ScriptedVerifier
from models.self_improve.multistep_arithmetic import LLMwVerifier_MultistepArithmetic
import re


class LLMEditor_MultistepArithmetic(LLMwVerifier_MultistepArithmetic, LLMEditor):
    FB_ERROR  = '[ERROR]'
    FB_NOEDIT = '[NOEDIT]'
    ...
    def _check_attempted_answer(self, llm_output, prev_answer, prev_feedback):
        ...
        parsed_attempt = ParsedAttempt_MultistepArithmetic(attempted_answer)
        parsed_prev_feedback = ParsedFeedback_MultistepArithmetic(prev_feedback)
    ...
    def get_filtered_verifier_feedback(self, question, attempt, correct_answer):
        if isinstance(self.verifier_model, ScriptedVerifier):
            ...
            if not last_step.endswith('.'):
                last_step += '.'
            numeric_outputs = re.findall(r'the answer is (.*)\.', last_step)
            if len(numeric_outputs) != 1:
                return "[ERROR]"
            try:
                final_num = float(numeric_outputs[0])
            except:
                print("Error in parsing numeric output:", numeric_outputs[0])
                return "[ERROR]"
            
            num_steps = len(attempt.split('\n')) - 1
            parsed_attempt = ParsedAttempt_MultistepArithmetic(attempt)
            if float(correct_answer) == float(final_num) and parsed_attempt.has_valid_content():
            ...
            generated_feedback = output['feedback']
            parsed_feedback = ParsedFeedback_MultistepArithmetic(generated_feedback)
    ...
    def get_to_edit_feedback(self, attempts: List[str], gen_feedbacks: List[str], llm_feedbacks: List[str], correct_ans: float):
        for gen_attempt, gen_feedback, llm_feedback in zip(attempts, gen_feedbacks, llm_feedbacks):
            ...
            parsed_llm_feedback = ParsedFeedback_MultistepArithmetic(llm_feedback)
            parsed_gen_feedback = ParsedFeedback_MultistepArithmetic(gen_feedback)
    ...
    def get_llm_answer_until_done(self, llm_prompt, correct_ans: float):
        ...
        while not done and updated < self.max_updates:
            ...
            parsed_attempted_answer = ParsedAttempt_MultistepArithmetic(attempted_answer)
            ...
            parsed_attempt_w_question = ParsedAttempt_MultistepArithmetic(attempted_answer_w_question)
            ...
            parsed_feedback = ParsedFeedback_MultistepArithmetic(feedback)
            parsed_prev_feedback = ParsedFeedback_MultistepArithmetic(prev_feedback)
            ...
            llm_prompt = {
                **llm_output,
                "feedback": feedback,
                "examples": EXAMPLES_MULTISTEP_ARITHMETIC_UPDATE  # EXAMPLES_UPDATE
            }
    ...
    def format_trajectory(self, trajectory: List[str]):
        ...
        out_text += f"Answer: Let's think step by step. {additional_info}\n{init_attempt}\n"
        ...
        for step in trajectory[3:]:
            step = step.strip()
            if is_attempt:
                out_text += f"Updated Answer: Let's think step by step. {additional_info}\n{step}\n"
            else:
        ...
        for p, p_ans, c_ans in zip(preds, pred_ans, correct_ans):
            try:
                ...
                if p is None:
                    ...
                    continue
                if float(p_ans) == float(c_ans) and ParsedAttempt_MultistepArithmetic(p).has_valid_content():
                    ori_p = p
