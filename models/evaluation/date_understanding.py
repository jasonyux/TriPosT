...
class DateUnderstandingEvaluator(Evaluator):
    def __init__(self, 
                 ...
                 dset_raw_file="data/raw/date_understanding_codex_rationales_cleaned.jsonl"):
        if split == 'test':
            split = 'validation'
    ...
    def _format_rationale(self, steps, final_answer):
        formatted_steps = []
        ...
        final_step = f"(Final response) So the answer is ({final_answer})."
        formatted_steps.append(final_step)
    ...
    def report_results(self, correct_rationales, correct_answers, pred_answers, noprint=False):
        result = {
            '1-2': {'total': 0, 'correct': 0},
            '3+': {'total': 0, 'correct': 0},
            'total': {'total': 0, 'correct': 0},
        }
        for correct_rationale, correct_answer, pred_answer in zip(correct_rationales, correct_answers, pred_answers):
            subtask = len(correct_rationale.split('\n')) - 1
            if subtask <= 2:
                subtask_str = '1-2'
            else:
                subtask_str = '3+'
    ...
    def report_short_results(self, correct_rationales, correct_answers, pred_answers):
        full_result = self.report_results(correct_rationales, correct_answers, pred_answers, noprint=True)
    ...
    def _get_batched_data(self):
        ...
        for data in self.dataset:
            if total_ == 0:
                break
            # evaluate
            num_steps = len(data['rationale'].split('\n')) - 1
            if self.subtask == '1-2' and num_steps > 2:
                continue
            elif self.subtask == '3+' and num_steps <= 3:
                continue
    ...
    def evaluate(self, model: GenerativeModel):
        ...
        for batch in batched_data:
            ...
            curr_res = self.report_short_results(correct_rationales, correct_answers, pred_answers)
            pbar.set_postfix(curr_res)
        pbar.close()
        
        # check answers
        result = self.report_results(correct_rationales, correct_answers, pred_answers)
        return correct_answers, pred_answers, preds, result
    

class DateUnderstandingTrainingEvaluator(DateUnderstandingEvaluator):
